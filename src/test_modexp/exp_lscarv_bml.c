...
void exp_bMl( const mrz_ctx_t* ctx, mrz_t r, const mrz_t x, const limb_t* y, int l_y, const mrz_t bl,const mrz_t b1 ) {
	mrz_t R[3];
	bool b, di;
	memcpy( R[0], bl, NL*SIZEOF( limb_t ) );
	memcpy( R[2], b1, NL*SIZEOF( limb_t ) );	
	mrz_mul( ctx, R[1], bl ,  x );
	for( int i = l_y - 1; i >= 0; i-- ) {
		for( int j = ( BITSOF( limb_t ) - 1 ); j >= 0; j-- ) {
			...
			mrz_mul( ctx, R[2],  R[2],  R[2]);
  		}
	}
	mrz_mul( ctx, R[0],  R[0],  R[2]);
...
int riscv_main() {
    ...
    putstr("\nExp Blinding Montgomery ladder \n");	
	...
	*/
	...
	uint8_t  barr[128]= {0xe8,0x58,0x77,0x07,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00, \
	...
	uint8_t  b1arr[128]={0x08,0xba,0xc3,0xe4,0x08,0x5c,0x21,0xfc,0x08,0x86,0x22,0xc3,0x18,0x26,0x90,0xa0, \
						 0xa5,0x92,0x08,0xee,0xe6,0xec,0xb6,0xa8,0xe6,0x7a,0xc1,0x37,0x16,0x91,0xf8,0xf8, \
					     0x9b,0xc0,0xbd,0xdc,0xd9,0x88,0xaf,0xe4,0x73,0x83,0x3b,0x98,0x1b,0x08,0xab,0x54, \
						 0xdb,0x97,0x89,0xab,0x9f,0x8e,0xfd,0x64,0xf1,0x3b,0x4e,0x98,0x0b,0xcd,0xbf,0xab, \
						 0x72,0xb9,0x0f,0x1c,0xed,0xec,0xa7,0x2c,0xe5,0x97,0xa6,0xf6,0x27,0x6c,0x63,0xa1, \
						 0xb7,0x71,0x3b,0xe8,0x02,0xc8,0x3a,0x7d,0xae,0xbd,0xa7,0x33,0xc6,0xa2,0x3f,0x9e, \
						 0xcc,0xdb,0xa2,0x19,0x09,0xb8,0xdd,0x19,0x44,0x41,0x8d,0x4e,0xc3,0x86,0x19,0x10, \
						 0x6b,0xf3,0x3a,0xc7,0x73,0x14,0xee,0xd5,0x6b,0x7f,0xad,0x88,0xe4,0xc0,0x24,0x92};
	...
	memcpy( b, barr, NL * SIZEOF( limb_t ) );
	memcpy( b1,b1arr,NL * SIZEOF( limb_t ) );
    ...
    for(int i=0; i<10; i++){
	    ...
	    mrz_mul( &ctx, b, b, ctx.rho_2 );
		mrz_mul( &ctx, b1,b1,ctx.rho_2 );
	    exp_bMl( &ctx, r, r, k, l_k , b,b1);
