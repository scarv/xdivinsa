...
void mrz_ca(mrz_t r, mrz_t c0, mrz_t c1, limb_t mask){
	for( int i = NL - 1; i >= 0; i-- ) { r[i] = (mask & c1[i])|((~mask) & c0[i]); }
}

void exp_ca( const mrz_ctx_t* ctx, mrz_t r, const mrz_t x, const limb_t* y, int l_y ) {
  mrz_t t,tt;
  limb_t mask;
