...
class SAFTFunc(object):
    ...
    def __init__(self,comms, scope, num_traces=10000, trace_channel="A"):
        ...
        self.set1_dat    = bytes.fromhex("01020304")
    ...
    def FuncTest(self):
        ...
        if(rsp):
            ...
            print("a+b   == 0x%s"%datout[0:4].hex())
            print("%s    == 0x%s"%(hex(int.from_bytes(datin[0:4],'big')+int.from_bytes(datin[4:8],'big')),datout[0:4].hex()))
        else:
